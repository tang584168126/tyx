...
namespace MySchoolDAL.Server
{
    ...
    public class ResultServer
    {
        ...
        /// 获取成绩列表
        /// </summary>
        /// <param name="objResBusiList">成绩实体扩展类对象</param>
        ...
        public List<ResultBusinessEntity> GetResBusiEntity(ResultBusinessEntity objResBusiEntity)
        {
            ...
            {
                //SQL命令
                string sqlText = " select a.StudentNo,b.StudentName,a.SubjectNo,c.SubjectName,a.StudentResult,a.ExamDate"
                                + " from Result as a"
                                + " left join Student as b on a.StudentNo = b,StudentNo"
                                + " left join Subject as c on c.SubjectNo = a.SubjectNo"
                                + " where 1=1";
                ...
                //获取成绩列表
                for (int i = 0; i < objDS.Tables[0].Rows[i].Count; i++)
                {
                    ...
                    objResBusiEntityTemp.ExamDate1 = Convert.ToDateTime(objDS.Tables[0].Rows[i]["ExamDate1"].ToString());
                    objResBusiEntityTemp.StudentResult1 = Convert.ToDecimal(objDS.Tables[0].Rows[i]["StudentResult1"].ToString());
