...
namespace BookShopMvc.Controllers
{
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Register([ModelBinder(typeof(UserBinder))]User user,string securityCode)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("nonCode","请输入验证码！");
                }
                if (securityCode != TempData["SecurityCode"])
                {
                    ModelState.AddModelError("userCode", "验证码输入错误！");
                }
            }
            else
            {
